        ...
        public static void LogTransaction(Iso8583Message msg, SourceNode srcNode = null, bool needReversal = false)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    Console.WriteLine("Transaction logged! -> ODE= " + transactionToLog.OriginalDataElement + " Amount: " + transactionToLog.Amount + " Date: " + transactionToLog.Date);

                }
                catch (Exception ex)
                {
                    MessageLogger.LogError("Transaction log failed! " + ex.Message + "   Inner Exception:  " + ex.InnerException);
                    Console.WriteLine("Transaction log failed! " + ex.Message + "   Inner Exception:  " + ex.InnerException);
                }
            }
            catch (Exception ex)
            {
