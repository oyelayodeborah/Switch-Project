...
namespace MySwitchProcessor
{
    public class Processor
    {
        ...
        public static void ListenerPeer(SourceNode sourceNode)     //create conn
        {
            ...
            Program.listenerPeers.Add(listener);
        ...
        public static void GetFinancialInstitution(Iso8583Message responseFromFEP)
        {
            string acquiringInstCode = Convert.ToString(responseFromFEP.Fields[32]);
            string forwardingInstCode = Convert.ToString(responseFromFEP.Fields[33]);
            var acqufinancialInst = new FinancialInstitutionRepository().GetByInstitutionCode(acquiringInstCode);
            var forwardingfinancialInst = new FinancialInstitutionRepository().GetByInstitutionCode(forwardingInstCode);
            try
            {
                if (acqufinancialInst == null)
                {
                    responseFromFEP.Fields.Add(39, ResponseCode.BANK_NOT_SUPPORTED); // ERROR
                }
                else if (acqufinancialInst==null && forwardingfinancialInst == null)
                {
                    responseFromFEP.Fields.Add(39, ResponseCode.BANK_NOT_SUPPORTED); // ERROR
                }
                else if (acqufinancialInst != null)
                {
                    Program.ConnectSinkNode(responseFromFEP);
                }
                else
                {
                    responseFromFEP.Fields.Add(39, ResponseCode.BANK_NOT_SUPPORTED); // ERROR
        ...
        public static Message SendMessage(Peer peer, Message msg)
        {
